...
APP_BUNDLE="${1:-dist/KeyForge.app}"
INFO="$APP_BUNDLE/Contents/Info.plist"
EXECUTABLE="$APP_BUNDLE/Contents/MacOS/KeyForge"
ICON="$APP_BUNDLE/Contents/Resources/KeyForge.icns"
...
[[ "$(plutil -extract CFBundleIdentifier raw "$INFO")" == "com.sababurduli.keyforge" ]] || fail "unexpected bundle identifier"
...
[[ "$(plutil -extract CFBundleIconFile raw "$INFO")" == "KeyForge" ]] || fail "unexpected icon metadata"
